...
var cartReducer = function cartReducer() {
  ...
  if (action.type === CURRENT_DOG) {
    var addedItem = state.dogs.find(function (item) {
  ...
  if (action.type === _actions_action_types_cart_actions__WEBPACK_IMPORTED_MODULE_11__["ADD_TO_CART"]) {
    ...
    if (_existed_item) {
      ...
      var _newTotal = state.total + _addedItem.price;
  ...
  if (action.type === _actions_action_types_cart_actions__WEBPACK_IMPORTED_MODULE_11__["ADD_QUANTITY_WITH_NUMBER"]) {
    ...
    var _existed_item2 = state.addedItems.find(function (item) {
      return action.id === item.id;
    });

    if (_existed_item2) {
      _addedItem2.quantity += action.qty;
      return _objectSpread({}, state, {
        total: state.total + _addedItem2.price * action.qty
      });
    } else {
      _addedItem2.quantity = action.qty; //calculating the total

      var _newTotal2 = state.total + _addedItem2.price * action.qty;

      return _objectSpread({}, state, {
        addedItems: [].concat(Object(_babel_runtime_corejs2_helpers_esm_toConsumableArray__WEBPACK_IMPORTED_MODULE_6__["default"])(state.addedItems), [_addedItem2]),
        total: _newTotal2
      });
    }
  }
  ...
  if (action.type === _actions_action_types_cart_actions__WEBPACK_IMPORTED_MODULE_11__["REMOVE_ITEM"]) {
    ...
    var _newTotal3 = state.total - itemToRemove.price * itemToRemove.quantity;
  ...
  if (action.type === _actions_action_types_cart_actions__WEBPACK_IMPORTED_MODULE_11__["SUB_QUANTITY"]) {
    var _addedItem4 = state.products.find(function (item) {
      return item.id === action.id;
    }); //if the qt == 0 then it should be removed


    if (_addedItem4.quantity === 1) {
      var _new_items2 = state.addedItems.filter(function (item) {
        return item.id !== action.id;
      });

      var _newTotal5 = state.total - _addedItem4.price;
      ...
      _addedItem4.quantity -= 1;

      var _newTotal6 = state.total - _addedItem4.price;
...
//# sourceMappingURL=_app.js.784326f26743987b920f.hot-update.js.map